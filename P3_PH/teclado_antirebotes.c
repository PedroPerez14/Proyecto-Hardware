...
static enum estado_keyboard que_tecla;	//Para saber qué botón hemos pulsado y saber cuál mirar a la hora de comprobar si se ha soltado
...
void teclaes_antirebotes_inicializar()
{
	...
	switch(maquina_estados)
	{
		...
		case encuestando:
			...
			{//Si es hora de encuestar al botón
				...
				if(cuenta_autoincremento == int_timer0_enable_autoincr)		//TODO poner autoincemento para tecla derecho
				{	//Tras mantener pulsado durante 1/3 de segundo
					//	autoincremento cada 180ms a partir de ahora
					cuenta_autoincremento = 0;
					hay_autoincremento = 1;
					if(que_tecla == tecla_0)
					{
						jugada_ev_izq();
					}
					else if(que_tecla == tecla_1)
					{
						jugada_ev_der();
					}
				}
				if(hay_autoincremento && cuenta_autoincremento == int_timer0_autoincr) 	//11 ticks de timer0 son 182.6 milisegs ~= 180 ms, para incrementar de forma auto
				{
					...
					//Lo hago como una llamada directa a jugada_por_teclaes porque la cola es concurrente y así se usa el mínimo posible

					if(que_tecla == button_iz)
					{
						jugada_ev_izq();
					}
					else if(que_tecla == button_dr)
					{
						jugada_ev_der();
					}
				}
...
void keyboard_ev_pulsacion(enum estado_button tecla)
